...
def get_adjlist(top):
    '''Calcualte adjacency list from mdtraj.Topology
    '''
...
def get_idx_pocket(traj, idx1, idx2, rcutoff=0.5):
    ''' Select atom indices from the binding pocket
    '''
    R_CLUSTER = rcutoff
    t = traj
...
def calc_idx_interface(traj, idx1, idx2, rcutoff=0.6):
    ''' Select atoms within a cutoff distance
    '''
    t = traj
...
def write_rest(int_idxs, r0s, k0s, fmt='tinker'):
    ''' Write restraints in Tinker format
    '''
...
def find_rotation_rest(fxyz, ligidx0, atomnames='CA', rcutoff=0.6, alpha=1.0, rest_rotbond=False, rotbond_only=False):
    ...
    # Eq. (14)
    assert len(k0s) == 6
    dgrest = RT*np.log(1662*8*np.pi**2.0*np.sqrt(np.prod(k0s*2.0)*RAD_IN_DEG**10.0) \
